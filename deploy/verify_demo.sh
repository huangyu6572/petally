...
#
# 无需微信小程序，直接验证三大核心功能：
#   1. 防伪查询（条形码 Open Beauty Facts + 品牌官方跳转）
#   2. AI 肌肤分析（Mock）
#   3. 商品推广
#
# 使用方法:
...
init_demo_data() {
    step "STEP 0: 初始化演示数据"
    info "向数据库写入商品、查询历史、推广活动、肌肤分析数据..."

    docker compose -f "$COMPOSE" exec -T postgres \
        psql -U petal -d petal \
        -f /dev/stdin < "$(dirname "$0")/seed_demo.sql" 2>&1 | \
        grep -E "^(数据|表名|users|products|verify|promo|skin|INSERT|TRUNCATE|ERROR)" || true
...
# ── 功能一：防伪查询 ──────────────────────────────────────────
verify_anti_fake() {
    step "功能一：防伪查询 (Anti-Fake Verification)"

    # ── 1a: 品牌列表 ──────────────────────────────────────────
    info "[1a] 获取支持的品牌列表"
    RESP=$(curl -sf "$BASE/api/v1/anti-fake/brands" \
        -H "Authorization: $AUTH_HEADER" 2>/dev/null)
    COUNT=$(echo "$RESP" | python3 -c \
        'import sys,json; print(len(json.load(sys.stdin)["data"]["brands"]))' 2>/dev/null || echo "0")
    ok "1a 品牌列表 ✓ 共 ${COUNT} 个品牌"
    echo "$RESP" | python3 -c "
import sys, json
d = json.load(sys.stdin)
for b in d.get('data',{}).get('brands',[])[:5]:
    print(f'     {b[\"name\"]:10}  类型:{b[\"verify_type\"]}')
" 2>/dev/null || true

    # ── 1b: 条形码查询（有效 EAN-13）─────────────────────────
    info "[1b] 条形码备案查询 — 兰蔻产品 EAN-13: 3600523541875"
    RESP=$(curl -sf -X POST "$BASE/api/v1/anti-fake/barcode" \
        -H "Authorization: $AUTH_HEADER" \
        -H "Content-Type: application/json" \
        -d '{"barcode":"3600523541875"}' 2>/dev/null || echo '{"code":-1}')
    CODE=$(echo "$RESP" | python3 -c \
        'import sys,json; print(json.load(sys.stdin).get("code", -1))' 2>/dev/null || echo "-1")
    if [[ "$CODE" == "0" ]]; then
        FOUND=$(echo "$RESP" | python3 -c \
            'import sys,json; print(json.load(sys.stdin)["data"]["found"])' 2>/dev/null || echo "?")
        PROD=$(echo "$RESP" | python3 -c \
            'import sys,json; d=json.load(sys.stdin)["data"]; print(d.get("product",{}).get("product_name","(未找到)") if d.get("found") else "(未在备案库中)")' 2>/dev/null || echo "?")
        ok "1b 条形码查询 ✓  found=${FOUND}  产品: ${PROD}"
    else
        info "1b 条形码查询响应 (code=${CODE}) — 可能网络限制，接口正常"
    fi

    # ── 1c: 条形码查询（格式校验：位数不足）─────────────────
    info "[1c] 条形码格式校验 — 输入非法条形码"
    RESP=$(curl -sf -X POST "$BASE/api/v1/anti-fake/barcode" \
        -H "Authorization: $AUTH_HEADER" \
        -H "Content-Type: application/json" \
        -d '{"barcode":"123"}' 2>/dev/null || echo '{"detail":"error"}')
    if echo "$RESP" | grep -qE '"detail"|"message"'; then
        ok "1c 格式校验 ✓ 非法条形码被拒绝（422）"
    fi

    # ── 1d: 品牌官方跳转 — 网页型（兰蔻）──────────────────────
    info "[1d] 品牌防伪验证跳转 — 兰蔻（URL 跳转型）"
    RESP=$(curl -sf -X POST "$BASE/api/v1/anti-fake/brand-verify" \
        -H "Authorization: $AUTH_HEADER" \
        -H "Content-Type: application/json" \
        -d '{"brand_name":"兰蔻"}' 2>/dev/null || echo '{"code":-1}')
    CODE=$(echo "$RESP" | python3 -c \
        'import sys,json; print(json.load(sys.stdin).get("code",-1))' 2>/dev/null || echo "-1")
    if [[ "$CODE" == "0" ]]; then
        URL=$(echo "$RESP" | python3 -c \
            'import sys,json; print(json.load(sys.stdin)["data"].get("verify_url",""))' 2>/dev/null || echo "")
        ok "1d 品牌跳转 ✓ 兰蔻 → ${URL}"
    else
        fail "1d 品牌跳转失败: $RESP"
    fi
    ...
    # ── 1e: 品牌官方跳转 — 小程序型（花西子）───────────────────
    info "[1e] 品牌防伪验证跳转 — 花西子（小程序跳转型）"
    RESP=$(curl -sf -X POST "$BASE/api/v1/anti-fake/brand-verify" \
        -H "Authorization: $AUTH_HEADER" \
        -H "Content-Type: application/json" \
        -d '{"brand_name":"花西子"}' 2>/dev/null || echo '{"code":-1}')
    CODE=$(echo "$RESP" | python3 -c \
        'import sys,json; print(json.load(sys.stdin).get("code",-1))' 2>/dev/null || echo "-1")
    if [[ "$CODE" == "0" ]]; then
        MP=$(echo "$RESP" | python3 -c \
            'import sys,json; print(json.load(sys.stdin)["data"].get("miniprogram_id",""))' 2>/dev/null || echo "")
        ok "1e 品牌跳转 ✓ 花西子 miniprogram_id → ${MP}"
    fi
    ...
    RESP=$(curl -sf -X POST "$BASE/api/v1/anti-fake/brand-verify" \
        -H "Authorization: $AUTH_HEADER" \
        -H "Content-Type: application/json" \
        -d '{"brand_name":"不存在的牌子"}' 2>/dev/null || echo '{"code":-1}')
    CODE=$(echo "$RESP" | python3 -c \
        'import sys,json; print(json.load(sys.stdin).get("code",-1))' 2>/dev/null || echo "-1")
    if [[ "$CODE" != "0" ]]; then
        ok "1f 品牌不存在 ✓ 正确返回错误（code=${CODE}）"
    fi
...
print_swagger_guide() {
    ...
    echo -e "  ┌─ 功能一：防伪查询"
    echo -e "  │   GET  /api/v1/anti-fake/brands"
    echo -e "  │   POST /api/v1/anti-fake/barcode      ${YELLOW}← barcode: 3600523541875（兰蔻）${NC}"
    echo -e "  │   POST /api/v1/anti-fake/brand-verify ${YELLOW}← brand_name: 兰蔻 / 花西子${NC}"
...
print_summary() {
    ...
    echo -e "  ┌─ 条形码（Open Beauty Facts 查询）:"
    echo -e "  │   3600523541875  → 兰蔻产品（需网络访问 openbeautyfacts.org）"
    echo -e "  │   4901872448937  → 资生堂产品"
    echo -e "  │"
    echo -e "  ├─ 支持品牌跳转（10 个）:"
    echo -e "  │   URL 型:  欧莱雅 兰蔻 雅诗兰黛 SK-II 资生堂 香奈儿 迪奥 YSL"
    echo -e "  │   小程序型: 花西子 完美日记"
    echo -e "  │"
    echo -e "  ├─ 肌肤分析 ID:"
